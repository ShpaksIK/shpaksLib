...
""" Модуль алгоритмов.
Множество видов алгоритмов представленно в этом модуле:
...
def selection_sort(arr: List, how_sort: str = "up", show_time: bool = False) -> List[int]: 
    ...
    for _ in range(len(arr)):
        if how_sort == "up":
            smallest = smallest_search(arr)
        elif how_sort == "down":
            smallest = biggest_search(arr)
        else:
...
def quick_sort(arr: List[int], show_time: bool = False) -> List[int]:
    ...
    show_time -- возвращать время выполнения функции (по умолчанию False)
    """
    # Быстрая сортировка через рекурсию (ограничение в стеке).
    if len(arr) <= 100:
        if len(arr) < 2:
            return arr
        else:
